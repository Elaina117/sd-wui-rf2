...
class Toprow:
    ...
    def create_submit_box(self):
        with gr.Row(elem_id=f"{self.id_part}_generate_box", elem_classes=["generate-box"] + (["generate-box-compact"] if self.is_compact else []), render=not self.is_compact) as submit_box:
            ...
            self.interrupt = gr.Button('中断', elem_id=f"{self.id_part}_interrupt", elem_classes="generate-box-interrupt", tooltip="直ちに、または現在のバッチ完了後に生成を終了")
            self.skip = gr.Button('スキップ', elem_id=f"{self.id_part}_skip", elem_classes="generate-box-skip", tooltip="現在のバッチ生成を停止し、次のバッチへ進む")
            ...
            self.submit = gr.Button('生成', elem_id=f"{self.id_part}_generate", variant='primary', tooltip="右クリックで無限生成メニュー")

            def interrupt_function():
                if not shared.state.stopping_generation and shared.state.job_count > 1 and shared.opts.interrupt_after_current:
                    shared.state.stop_generating()
                    gr.Info("この画像の生成後に停止します。直ちに停止するにはもう一度クリックしてください。")
                else:
    ...
    def create_tools_row(self):
        with gr.Row(elem_id=f"{self.id_part}_tools"):
            ...
            self.paste = ToolButton(value=paste_symbol, elem_id="paste", tooltip="プロンプトが空の場合、最後の生成パラメータまたはプロンプトを読み込む")
            self.clear_prompt_button = ToolButton(value=clear_prompt_symbol, elem_id=f"{self.id_part}_clear_prompt", tooltip="プロンプトを消去")
            self.apply_styles = ToolButton(value=ui_prompt_styles.styles_materialize_symbol, elem_id=f"{self.id_part}_style_apply", tooltip="選択したスタイルをプロンプトに適用")

            if self.is_img2img:
                self.button_interrogate = ToolButton('📎', tooltip='CLIP解析 - 画像からテキストを生成しプロンプトに入力', elem_id="interrogate")
                self.button_deepbooru = ToolButton('📦', tooltip='DeepBooru解析 - 画像からタグを生成しプロンプトに入力', elem_id="deepbooru")

            self.restore_progress_button = ToolButton(value=restore_progress_symbol, elem_id=f"{self.id_part}_restore_progress", visible=False, tooltip="生成結果を復元")
