...
def create_ui():
                    ...
                    if category == "prompt":
                        ...
                        with FormRow():
                            ...
                            with gr.Column(elem_id="txt2img_dimensions_row", scale=1, elem_classes="dimensions-tools"):
                                res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="txt2img_res_switch_btn", tooltip="幅と高さを入れ替え")

                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="txt2img_column_batch"):
                                    batch_count = gr.Slider(minimum=1, step=1, label='バッチ回数', value=1, elem_id="txt2img_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=50, step=1, label='バッチサイズ', value=1, elem_id="txt2img_batch_size")

                    elif category == "cfg":
                        with gr.Row():
                            cfg_scale = gr.Slider(minimum=0.0, maximum=30.0, step=0.1, label='CFGスケール', value=7.0, elem_id="txt2img_cfg_scale")
                        ...
                        with gr.Row(elem_id="txt2img_accordions", elem_classes="accordions"):
                            with InputAccordion(False, label="高解像度補助 (Hires. fix)", elem_id="txt2img_hr") as enable_hr:
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row1", variant="compact"):
                                    ...
                                    hr_second_pass_steps = gr.Slider(minimum=0, maximum=150, step=1, label='Hiresステップ数', value=0, elem_id="txt2img_hires_steps")
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row2", variant="compact"):
                                    hr_scale = gr.Slider(minimum=1.0, maximum=4.0, step=0.05, label="拡大倍率", value=2.0, elem_id="txt2img_hr_scale")
                                    hr_resize_x = gr.Slider(minimum=0, maximum=2048, step=8, label="幅をリサイズ(0で自動)", value=0, elem_id="txt2img_hr_resize_x")
                                    hr_resize_y = gr.Slider(minimum=0, maximum=2048, step=8, label="高さをリサイズ(0で自動)", value=0, elem_id="txt2img_hr_resize_y")

                                with FormRow(elem_id="txt2img_hires_fix_row_cfg", variant="compact"):
                                    hr_cfg = gr.Slider(minimum=0.0, maximum=30.0, step=0.1, label="Hires CFGスケール", value=0.0, elem_id="txt2img_hr_cfg")
                                ...
                                with FormRow(elem_id="txt2img_hires_fix_row3", variant="compact", visible=opts.hires_fix_show_sampler) as hr_sampler_container:
                                    ...
                                    hr_sampler_name = gr.Dropdown(label='Hiresサンプリング方法', elem_id="hr_sampler", choices=["Use same sampler"] + sd_samplers.visible_sampler_names(), value="Use same sampler")
                                    hr_scheduler = gr.Dropdown(label='Hiresスケジュールタイプ', elem_id="hr_scheduler", choices=["Use same scheduler"] + [x.label for x in sd_schedulers.schedulers], value="Use same scheduler")

                                with FormRow(elem_id="txt2img_hires_fix_row4", variant="compact", visible=opts.hires_fix_show_prompts) as hr_prompts_container:
                                    with gr.Column(scale=80):
                                        with gr.Row():
                                            hr_prompt = gr.Textbox(label="Hiresプロンプト", elem_id="hires_prompt", show_label=False, lines=3, placeholder="Prompt for hires fix pass.\nLeave empty to use the same prompt as in first pass.", elem_classes=["prompt"])
                                    with gr.Column(scale=80):
                                        with gr.Row():
                                            hr_negative_prompt = gr.Textbox(label="Hiresネガティブプロンプト", elem_id="hires_neg_prompt", show_label=False, lines=3, placeholder="Negative prompt for hires fix pass.\nLeave empty to use the same negative prompt as in first pass.", elem_classes=["prompt"])
    ...
    with gr.Blocks(analytics_enabled=False) as img2img_interface:
        ...
        with gr.Tab("生成(Generation)", id="img2img_generation") as img2img_generation_tab, ResizeHandleRow(equal_height=False):
            with ExitStack() as stack:
                ...
                for category in ordered_ui_categories():
                    ...
                    if category == "image":
                        ...
                        with FormRow():
                            resize_mode = gr.Radio(label="リサイズモード", elem_id="resize_mode", choices=["リサイズのみ", "切り抜きとリサイズ", "リサイズと埋め込み", "リサイズのみ (Latent upscale)"], type="index", value="Just resize")
                        ...
                        with FormRow():
                            with gr.Column(elem_id="img2img_column_size", scale=4):
                                ...
                                with gr.Tabs(elem_id="img2img_tabs_resize"):
                                    with gr.Tab(label="Resize to", id="to", elem_id="img2img_tab_resize_to") as tab_scale_to:
                                        with FormRow():
                                            ...
                                            with gr.Column(elem_id="img2img_dimensions_row", scale=1, elem_classes="dimensions-tools"):
                                                res_switch_btn = ToolButton(value=switch_values_symbol, elem_id="img2img_res_switch_btn", tooltip="幅と高さを入れ替え")
                                    ...
                                    with gr.Tab(label="Resize by", id="by", elem_id="img2img_tab_resize_by") as tab_scale_by:
                                        scale_by = gr.Slider(minimum=0.05, maximum=4.0, step=0.05, label="倍率(Scale)", value=1.0, elem_id="img2img_scale")
                            ...
                            if opts.dimensions_and_batch_together:
                                with gr.Column(elem_id="img2img_column_batch"):
                                    batch_count = gr.Slider(minimum=1, step=1, label='バッチ回数', value=1, elem_id="img2img_batch_count")
                                    batch_size = gr.Slider(minimum=1, maximum=50, step=1, label='バッチサイズ', value=1, elem_id="img2img_batch_size")
                        ...
                        with gr.Row():
                            ...
                            image_cfg_scale = gr.Slider(minimum=0, maximum=3.0, step=0.05, label='画像CFGスケール', value=1.5, elem_id="img2img_image_cfg_scale", visible=False)
                        ...
                        with FormGroup(elem_id="inpaint_controls", visible=False) as inpaint_controls:
                            with FormRow():
                                mask_blur = gr.Slider(label='マスクのぼかし', minimum=0, maximum=64, step=1, value=4, elem_id="img2img_mask_blur")
                                mask_alpha = gr.Slider(label="マスクの透明度", visible=False, elem_id="img2img_mask_alpha")

                            with FormRow():
                                inpainting_mask_invert = gr.Radio(label='マスクモード', choices=['マスク領域を描画', 'マスク領域以外を描画'], value='Inpaint masked', type="index", elem_id="img2img_mask_mode")

                            with FormRow():
                                inpainting_fill = gr.Radio(label='マスクされたコンテンツ', choices=['埋める(fill)', '元画像(original)', '潜在ノイズ(latent noise)', '無(latent nothing)'], value='original', type="index", elem_id="img2img_inpainting_fill")

                            with FormRow():
                                with gr.Column():
                                    inpaint_full_res = gr.Radio(label="インペイント領域", choices=["画像全体", "マスクのみ"], type="index", value="Whole picture", elem_id="img2img_inpaint_full_res")

                                with gr.Column(scale=4):
                                    inpaint_full_res_padding = gr.Slider(label='マスク領域のパディング(px)', minimum=0, maximum=256, step=4, value=32, elem_id="img2img_inpaint_full_res_padding")
    ...
    with gr.Blocks(analytics_enabled=False) as train_interface:
        ...
        with ResizeHandleRow(variant="compact", equal_height=False):
            with gr.Tabs(elem_id="train_tabs"):
                ...
                with gr.Tab(label="Embeddingを作成", id="create_embedding"):
                    new_embedding_name = gr.Textbox(label="Name", elem_id="train_new_embedding_name")
                    initialization_text = gr.Textbox(label="初期化テキスト", value="*", elem_id="train_initialization_text")
                    nvpt = gr.Slider(label="トークンごとのベクトル数", minimum=1, maximum=75, step=1, value=1, elem_id="train_nvpt")
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            create_embedding = gr.Button(value="Embeddingを作成", variant='primary', elem_id="train_create_embedding")

                with gr.Tab(label="Hypernetworkを作成", id="create_hypernetwork"):
                    ...
                    with gr.Row():
                        ...
                        with gr.Column():
                            create_hypernetwork = gr.Button(value="Hypernetworkを作成", variant='primary', elem_id="train_create_hypernetwork")
                ...
                with gr.Tab(label="Train", id="train"):
                    ...
                    with gr.Row():
                        train_embedding = gr.Button(value="Embeddingを学習", variant='primary', elem_id="train_train_embedding")
                        interrupt_training = gr.Button(value="中断", elem_id="train_interrupt_training")
                        train_hypernetwork = gr.Button(value="Hypernetworkを学習", variant='primary', elem_id="train_train_hypernetwork")
